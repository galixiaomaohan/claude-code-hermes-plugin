...
export type AutoSkillCreatorOptions = {
  targetDir?: 'project' | 'user'
  projectCwd?: string
}

const DEFAULT_MODEL = 'claude-sonnet-4-6'

function extractToolNamesFromMessages(messages: Message[]): string[] {
  const names = new Set<string>()
  for (const m of messages) {
    if (m.type !== 'assistant') continue
    const assistant = m as { message?: { content?: unknown } }
...
function extractUserMessages(messages: Message[]): string[] {
  ...
  for (const m of messages) {
    ...
    const user = m as { message?: { content?: unknown } }
...
export class AutoSkillCreator {
  ...
  async analyzeAndCreate(
    messages: Message[],
    opts: AutoSkillCreatorOptions = {},
  ): Promise<{ skillPath: string; frontmatter: string } | null> {
...
Respond ONLY with the SKILL.md content (including frontmatter delimiters). Do not wrap in markdown code blocks.`
    ...
    try {
      ...
      skillMarkdown = textBlocks.map(b => (b as { text: string }).text).join('\n').trim()
    } catch (e) {
...
export function getAutoSkillCreator(sideQueryFn?: SideQueryFunction): AutoSkillCreator {
