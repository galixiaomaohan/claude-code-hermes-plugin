...
export class SkillHubClient {
    ...
    async search(query, limit = 10) {
        try {
            const resp = await this.fetchWithTimeout(`${this.baseUrl}/api/v1/skills/search?q=${encodeURIComponent(query)}&limit=${limit}`, {
                method: 'GET',
    ...
    async listSkills(limit = 50) {
        try {
            const resp = await this.fetchWithTimeout(`${this.baseUrl}/api/v1/skills?limit=${limit}`, {
                method: 'GET',
                headers: { Accept: 'application/json' },
            });
            if (!resp.ok) {
    ...
    async install(identifier, targetDir) {
        try {
            const resp = await this.fetchWithTimeout(`${this.baseUrl}/api/v1/skills/${encodeURIComponent(identifier)}/download`, {
                method: 'GET',
                headers: { Accept: 'application/json' },
            });
            if (!resp.ok) {
