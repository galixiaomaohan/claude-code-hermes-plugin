...
export class SessionStore {
  ...
  private runMigrations(): void {
    ...
    const ftsCheck = this.db.prepare<[], { name: string }>(`SELECT name FROM sqlite_master WHERE type='table' AND name='messages_fts';`)
    ...
    const summaryFtsCheck = this.db.prepare<[], { name: string }>(`SELECT name FROM sqlite_master WHERE type='table' AND name='sessions_fts';`)
    ...
    `)
    const versionRow = this.db.prepare<[], { version: number }>(`SELECT version FROM schema_version LIMIT 1;`).get()
  ...
  saveSession(
    ...
    try {
      ...
      const insertSession = this.db.prepare(
        `INSERT OR REPLACE INTO sessions(id, title, summary, cwd, parent_session_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?);`,
      )
      const existing = this.db.prepare<[string], { created_at: number }>(`SELECT created_at FROM sessions WHERE id = ?;`).get(sessionId)
  ...
  searchSessions(query: string, limit = 10): Array<SessionRow & { rank: number }> {
    try {
      const sql = this.db.prepare<
        [string, number],
        { id: string; title: string | null; summary: string | null; cwd: string | null; parent_session_id: string | null; created_at: number; updated_at: number; rank: number }
      >(`
        SELECT s.*, rank
        FROM sessions_fts
      ...
      try {
        const fallback = this.db.prepare<
          ...
          { id: string; title: string | null; summary: string | null; cwd: string | null; parent_session_id: string | null; created_at: number; updated_at: number }
        >(`
  ...
  getSessionSummary(sessionId: string): string | null {
    try {
      const row = this.db.prepare<[string], { summary: string | null }>(`SELECT summary FROM sessions WHERE id = ?;`).get(sessionId)
  ...
  getRelevantSessions(currentCwd: string, query: string, limit = 5): Array<SessionRow & { rank: number }> {
    try {
      ...
      const map = new Map<string, SessionRow & { rank: number }>()
...
export async function initSharedSessionStore(): Promise<void> {
