...
import type { HermesCommandMeta, HermesSkillDefinition, HermesCommandContext, Message } from './types.js'
import { getAutoSkillCreator } from './modules/AutoSkillCreator.js'
import { getSkillOptimizer } from './modules/SkillOptimizer.js'
import { SkillHubClient, getSkillHubClient } from './modules/SkillHubClient.js'
...
async function skillsHubPrompt(args: string): Promise<Array<{ type: 'text'; text: string }>> {
...
async function createSkillPrompt(_args: string, context: HermesCommandContext): Promise<Array<{ type: 'text'; text: string }>> {
  ...
  if (!result) {
    return textBlock(
      'Could not auto-create a skill from the recent conversation. Try providing more context or examples.',
    )
  }

  return textBlock(
    `Auto-created skill at **${result.skillPath}**.\n\nFrontmatter:\n\`\`\`yaml\n${result.frontmatter}\n\`\`\`\n\nThe skill is now available in the project skills directory.`,
  )
}

async function optimizeSkillPrompt(args: string): Promise<Array<{ type: 'text'; text: string }>> {
  ...
  if (!should) {
    return textBlock(
      `Skill "${skillName}" does not have enough usage data to optimize yet (threshold: 10 calls).`,
    )
  }
...
async function memorySearchPrompt(args: string): Promise<Array<{ type: 'text'; text: string }>> {
  ...
  const list = sessions
    .map(
      (s, i) =>
        `${i + 1}. **${s.id}** — ${new Date(s.updated_at).toLocaleString()}\n   ${s.summary || '(no summary)'}`,
    )
    .join('\n\n')
...
async function compressContextPrompt(_args: string, context: HermesCommandContext): Promise<Array<{ type: 'text'; text: string }>> {
...
async function logTrajectoryPrompt(_args: string, context: HermesCommandContext): Promise<Array<{ type: 'text'; text: string }>> {
  const logger = getTrajectoryLogger()
  const messages = context.messages || []
  const model = context.model || 'unknown'

  await logger.log(
    context.sessionId || 'manual',
    model,
    messages,
    [],
    'Manual trajectory log triggered via /hermes:log-trajectory',
  )

  return textBlock('Current conversation trajectory has been logged to the Hermes training bridge.')
}

async function statusPrompt(): Promise<Array<{ type: 'text'; text: string }>> {
...
const promptMap: Record<string, (args: string, context: HermesCommandContext) => Promise<Array<{ type: 'text'; text: string }>>> = {
...
export const hermesCommandsMeta: HermesCommandMeta[] = [
  {
...
export const hermesSkillDefinitions: HermesSkillDefinition[] = hermesCommandsMeta.map(meta => ({
  name: meta.name,
...
export function registerHermesPlugin(registerSkill: (meta: HermesCommandMeta) => void): void {
